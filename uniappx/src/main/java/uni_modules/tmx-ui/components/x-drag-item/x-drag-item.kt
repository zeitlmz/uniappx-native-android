...
open class GenUniModulesTmxUiComponentsXDragItemXDragItem : VueComponent {
    ...
    open fun gen_getNodes_fn() {
        uni_createSelectorQuery().`in`(this).select(".xDragItem").boundingClientRect().exec(fun(ret){
            var nodeinfo = ret[0] as NodeInfo
            this.cellHeight = nodeinfo.height!!
            this.cellWidth = nodeinfo.width!!
            this.pushChildren(nodeinfo)
    ...
    open fun gen_pushChildren_fn(node: NodeInfo) {
        var parent: XDragComponentPublicInstance? = null
        try {
            parent = this.`$parent` as XDragComponentPublicInstance
        }
        ...
        if (parent != null) {
            parent!!.addItem(CHILDREN_INFO(id = this.id, index = this.order, oldindex = this.order, ele = this, node = node))
        }
    }
    ...
    companion object {
        ...
        var props = normalizePropsOptions(utsMapOf("order" to utsMapOf("type" to "Number", "default" to 0, "required" to true)))
