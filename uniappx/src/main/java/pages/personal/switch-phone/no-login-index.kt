...
open class GenPagesPersonalSwitchPhoneNoLoginIndex : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesPersonalSwitchPhoneNoLoginIndex) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesPersonalSwitchPhoneNoLoginIndex
            val _cache = __ins.renderCache
            val router = uni_useKuxRouter()
            val globalData = inject("globalData") as GlobalDataType
            val reqData = reactive<PHONE_INFO2>(PHONE_INFO2(oldPhone = "", newPhone = "", newPassword = "", code = ""))
            ...
            val onGetCode = fun(){
                ...
                if (reqData.newPhone == null || reqData.newPhone == "") {
                    showTips1(XTIPS_TYPE(title = "新手机号不能为空", iconCode = "error", iconColor = "red", titleColor = "red", position = "bottom"))
                    return
                }
                showLoading(XLOADINGS_TYPE(title = "验证码发送中"))
                sendChangePhoneValid(reqData.newPhone as String).then(fun(res: Response){
                    if (res.code == 200) {
            ...
            val toCheckSecondPasswordByPhoneNumber = fun(){
                if (reqData.newPassword == null || reqData.newPassword == "" || reqData.newPassword.length != 6) {
                    showToast1(XTOAST_TYPE(title = "请输入六位数的二级密码", iconCode = "error"))
                    return
                }
                if (reqData.oldPhone == null || reqData.oldPhone == "") {
                    showToast1(XTOAST_TYPE(title = "请输入旧手机号", iconCode = "error"))
                    return
                }
                checkSecondPasswordByPhoneNumber(reqData.oldPhone, md5(reqData.newPassword)).then(fun(res: Response){
                    console.log("checkSecondPasswordByPhoneNumber=", res)
                    if (res.code == 200 && res.data == true) {
                        onGetCode()
                    } else {
                        showToast1(XTOAST_TYPE(title = "二级密码错误", iconCode = "error"))
                    }
                }
                )
            }
            val toSwitchCheck = fun(){
                ...
                checkPhoneNumber(reqData.newPhone).then(fun(res: Response){
                    console.log("checkPhoneNumber=", res)
                    if (res.code == 200) {
                        toCheckSecondPasswordByPhoneNumber()
                    } else {
            ...
            val toSwitch = fun(){
                ...
                if (reqData.newPhone == null || reqData.newPhone == "") {
                    showToast1(XTOAST_TYPE(title = "请输入新手机号", iconCode = "error"))
                    return
                }
                if (reqData.code == null || reqData.code == "") {
                    showToast1(XTOAST_TYPE(title = "请输入验证码", iconCode = "error"))
                    return
                }
                changePhoneNumber(reqData.oldPhone, reqData.newPhone, reqData.code).then(fun(res: Response){
                    if (res.code == 200) {
                        showToast1(XTOAST_TYPE(title = "修改成功，请使用新手机号登录", iconCode = "success", iconColor = "green", titleColor = "green"))
                        setTimeout(fun(){
