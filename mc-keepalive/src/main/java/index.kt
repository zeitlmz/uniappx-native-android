...
open class FloatingWindowManager {
    ...
    public open fun showFloatingWindow(context: Context): Unit {
        ...
        try {
            ...
            this.layoutParams = WindowManagerLayoutParams(
                ...
                WindowManagerLayoutParams.FLAG_NOT_FOCUSABLE or WindowManagerLayoutParams.FLAG_NOT_TOUCH_MODAL or WindowManagerLayoutParams.FLAG_LAYOUT_IN_SCREEN,
    ...
    private fun createFloatingView(context: Context): LinearLayout {
        ...
        val handleTouch: ViewTouchType =
            fun(context: Context, view: View, event: MotionEvent): Boolean {
    ...
    private fun animateFloatingViewEntry(): Unit {
        ...
        val alphaAnimator =
            ObjectAnimator.ofFloat(this.floatingView!!, "alpha", 0.0.toFloat(), 1.0.toFloat())
        alphaAnimator.setDuration(300.toLong())
        val scaleXAnimator =
            ObjectAnimator.ofFloat(this.floatingView!!, "scaleX", 0.5.toFloat(), 1.0.toFloat())
        scaleXAnimator.setDuration(300.toLong())
        val scaleYAnimator =
            ObjectAnimator.ofFloat(this.floatingView!!, "scaleY", 0.5.toFloat(), 1.0.toFloat())
    ...
    private fun setNotificationAndForeground(): Unit {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val notificationChannel =
                NotificationChannel(CHANNEL_ID, "保活服务", NotificationManager.IMPORTANCE_LOW)
            ...
            val notificationBuilder =
                Notification.Builder(context, CHANNEL_ID).setAutoCancel(false).setOngoing(true)
                    .setContentTitle("每橙车主").setContentText("正在后台运行")
                    .setSmallIcon(R.drawable.ic_menu_rotate)
                    .setCategory(Notification.CATEGORY_EVENT)
            val notification = notificationBuilder.build()
            super.startForeground(1001, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC)
        }
    }
